...
for ir = 1:length(which_refs)
    ...
    prob_stats = nan(2,8); % engel, ilae; mean good, std good, mean bad, std bad, df, tstat, p, cohen d
    ...
    for io = 1:2
        ...
        stats = unpaired_plot_tle(concordant_lat_scores(good_outcome),concordant_lat_scores(bad_outcome),...
            {good_outcome_text,bad_outcome_text},{'Probability of','concordant laterality'},'para');
        set(gca().Children(3),'MarkerSize',10)
        set(gca().Children(4),'MarkerSize',10)
        ...
        prob_stats(io,:) = [stats.means(1) stats.sd(1) stats.means(2) stats.sd(2),...
            stats.df stats.tstat stats.p cohen_d];
    ...
    if ir == 1
        ...
        fprintf(fid,[' We hypothesized that patients with a good surgical outcome would have a '...
            ...
            'Engel outcomes (%1.2f (%1.2f)) (<i>t</i>(%d) = %1.1f, %s, <i>d</i> = %1.2f) (Fig. 6B), and '...
            'in patients with good ILAE outcomes (%1.2f (%1.2f)) than '...
            'in patients with poor ILAE outcomes (%1.2f (%1.2f)) (<i>t</i>(%d) = %1.1f, %s, <i>d</i> = %1.2f) (Fig. 6D). '...
